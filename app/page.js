...
/* Parallax: devuelve desplazamiento de scroll optimizado con rAF */
...
/* Tarjetas de fondo — más densas y variadas */
const GHOST_CARDS = [
...
export default function Home() {
  ...
  return (
    <main className="page">
      {/* Capa de tarjetas de fondo para toda la zona superior */}
      <div className="ghost-layer">
        {GHOST_CARDS.map((c, i) => (
          <div
            key={i}
            className={`ghostcard gc-${i} size-${c.size}`}
            style={{ transform: `translateY(${-scrollY * c.speed}px)` }}
          >
      ...
      <section className="hero">
        ...
        <div className="hero-content">
          ...
          <h1 className="hero-title">
            <span className={`anim title-line ${loaded ? "in" : ""}`} style={{ transitionDelay: "250ms" }}>
              ¿Sabes cuánto
            </span>
            <span className={`anim title-line grad ${loaded ? "in" : ""}`} style={{ transitionDelay: "400ms" }}>
              te corresponde?
            </span>
          </h1>

          <p className={`anim hero-sub ${loaded ? "in" : ""}`} style={{ transitionDelay: "580ms" }}>
            Calcula lo que la ley mexicana dice que te pertenece. Gratis, sin
            registro y sin tarjeta — en menos de un minuto.
          </p>

          <div className={`anim stats ${loaded ? "in" : ""}`} style={{ transitionDelay: "720ms" }} ref={statsRef}>
            <div className="stat">
              <div className="stat-num blue">{Math.round(c1)}</div>
              <div className="stat-label">Calculadoras</div>
            </div>
            <div className="stat divider">
              <div className="stat-num dark">1 min</div>
              <div className="stat-label">Sin esperas</div>
            </div>
            <div className="stat">
              <div className="stat-num green" style={{ fontSize: "clamp(22px, 6vw, 30px)" }}>🔒</div>
              <div className="stat-label">Privado y anónimo</div>
            </div>
          </div>

          <div className={`anim cta-row ${loaded ? "in" : ""}`} style={{ transitionDelay: "860ms" }}>
            <a href="#calculadoras" className="btn-primary"
              style={{ background: "#1847f0", color: "#fff", fontSize: 15, fontWeight: 700, padding: "14px 26px", borderRadius: 100, textDecoration: "none", boxShadow: "0 6px 20px rgba(24,71,240,0.25)" }}>
      ...
      <section id="calculadoras" className="section" ref={pillsRef}>
        <div className={`section-head ${pillsVisible ? "in" : ""}`}>
          ...
          <h2 className="section-title">
            Cuatro temas. <span className="grad">Una respuesta clara.</span>
          </h2>
          <p className="section-sub">
            Cada calculadora está basada en una ley distinta. Elige la que
            necesites — todas son gratis.
          </p>
        </div>

        <div className="cards">
          <CalcCard delay={0} visible={pillsVisible} href="/finiquito" emoji="💼"
            title="Finiquito Laboral" desc="¿Renunciaste o te despidieron? Calcula lo que te corresponde." tag="LFT vigente" featured />
          <CalcCard delay={90} visible={pillsVisible} href="/pension-alimenticia" emoji="👧"
            title="Pensión Alimenticia" desc="Estima cuánto le corresponde a tus hijos por ley." tag="Código Civil" />
          <CalcCard delay={180} visible={pillsVisible} href="/pension-imss" emoji="🏛️"
            title="Pensión IMSS" desc="Calcula tu jubilación estimada según tus semanas cotizadas." tag="Ley del Seguro Social" />
          <CalcCard delay={270} visible={pillsVisible} href="/herencias" emoji="📋"
            title="Herencias" desc="¿Quién hereda y cuánto si no hay testamento?" tag="Código Civil" />
        </div>
      </section>
      ...
      <section id="confianza" className="trust-section" ref={trustRef}>
        <div className={`section-head ${trustVisible ? "in" : ""}`}>
          ...
          <h2 className="section-title light">
            Gratis de verdad. <span className="grad-light">Sin trucos.</span>
          </h2>
        ...
        <Link href="/finiquito" className="btn-primary big"
          style={{ display: "inline-block", background: "#1847f0", color: "#fff", fontSize: 16, fontWeight: 700, padding: "16px 32px", borderRadius: 100, textDecoration: "none", marginTop: 28, boxShadow: "0 6px 20px rgba(24,71,240,0.25)" }}>
      ...
      <footer className="footer">
        ...
        <div className="footer-legal">
          Los resultados son estimaciones informativas y no constituyen
          asesoría legal profesional.
          <br />© 2026 CalculadoraMX — Siempre gratis.
        </div>
      </footer>
      ...
      <style jsx>{`
        ...
        /* Tarjetas de fondo - cubren toda la zona superior */
        ...
        .gc-0 { top: 90px; left: -40px; }
        .gc-1 { top: 180px; right: -50px; }
        .gc-2 { top: 380px; left: -45px; }
        .gc-3 { top: 520px; right: -40px; }
        .gc-4 { top: 60px; left: 30%; opacity: 0.4; }
        .gc-5 { top: 300px; right: 24%; opacity: 0.4; }
        .gc-6 { top: 680px; left: 8%; opacity: 0.42; }
        .gc-7 { top: 880px; left: 30%; opacity: 0.4; }
        .gc-8 { top: 140px; left: 12%; opacity: 0.42; }
        .gc-9 { top: 460px; right: 6%; opacity: 0.4; }
        .gc-10 { top: 760px; right: 22%; opacity: 0.4; }
        .gc-11 { top: 1000px; left: 10%; opacity: 0.38; }
        .gc-12 { top: 1080px; right: 12%; opacity: 0.38; }
        .gc-13 { top: 600px; left: 38%; opacity: 0.36; }
        ...
        .stat-num.blue { color: #1847f0; }
        .stat-num.dark { color: #0a0f1e; }
    ...
    <section className="hiw" ref={ref}>
      <div className={`hiw-head ${visible ? "in" : ""}`}>
        ...
        <h2 className="hiw-title">
          De la duda al resultado <span className="grad">en 1 minuto.</span>
        </h2>
      ...
      <style jsx>{`
        ...
        .step {
          position: relative; background: #fff; border: 1px solid #eef2ff;
          border-radius: 18px; padding: 24px 22px; overflow: hidden;
        ...
        .step-watermark {
          ...
          font-size: 110px; font-weight: 800; color: #eef2ff;
        ...
        @media (min-width: 720px) {
          ...
          .hiw-grid { flex-direction: row; }
          .step { flex: 1; }
        }
        @media (prefers-reduced-motion: reduce) {
          .hiw-head, .step { transition: none !important; opacity: 1 !important; transform: none !important; }
        }
      `}</style>
    </section>
  );
  ...
  return (
    <Link href={href} style={{ textDecoration: "none" }}>
      <div className={`cc ${visible ? "in" : ""}`} style={{ transitionDelay: `${delay}ms` }}>
        ...
        <style jsx>{`
          .cc {
            background: #fff; border: 1px solid #eef2ff; border-radius: 18px;
            padding: 20px; cursor: pointer;
            opacity: 0; transform: translateY(20px);
            transition: opacity 0.6s cubic-bezier(0.22,1,0.36,1), transform 0.6s cubic-bezier(0.22,1,0.36,1), box-shadow 0.25s ease, border-color 0.2s ease;
          }
          ...
          .cc-btn {
            background: #1847f0; color: #fff; font-size: 14px; font-weight: 700;
